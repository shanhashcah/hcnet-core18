...
{
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto trustLine = hcnet::loadTrustLine(ltx, getPublicKey(), asset);
    REQUIRE(trustLine);
...
TestAccount::getAvailableBalance() const
{
    ...
    return hcnet::getAvailableBalance(header, entry);
}
                                    ...
                                    xdr::xvector<Claimant, 10> const& claimants)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    auto entry = hcnet::loadClaimableBalance(ltx, returnedBalanceID);
    REQUIRE(entry);
...
TestAccount::claimClaimableBalance(ClaimableBalanceID const& balanceID)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(!hcnet::loadClaimableBalance(ltx, balanceID));
}
