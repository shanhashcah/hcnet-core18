    ...
    if (!cfgs[instanceNumber])
    {
        if (gTestRoots.empty())
        {
            gTestRoots.emplace_back(
                fmt::format("hcnet-core-test-{}", gBaseInstance));
        }
...
runTest(CommandLineArgs const& args)
{
    ...
    // instances of hcnet-core running at the same time with the same base
    // instance
    auto logFile = fmt::format("hcnet{}.log", gBaseInstance);
    Logging::setLoggingToFile(logFile);
    Logging::setLogLevel(logLevel, nullptr);

    LOG_INFO(DEFAULT_LOG, "Testing hcnet-core {}", HCNET_CORE_VERSION);
