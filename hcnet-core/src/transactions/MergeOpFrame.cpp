...
MergeOpFrame::doApplyBeforeV16(AbstractLedgerTxn& ltx)
{
    ...
    auto otherAccount =
        hcnet::loadAccount(ltx, toAccountID(mOperation.body.destination()));
    if (!otherAccount)
    {
...
MergeOpFrame::doApplyFromV16(AbstractLedgerTxn& ltx)
{
    ...
    if (!hcnet::loadAccount(ltx, toAccountID(mOperation.body.destination())))
    {
        innerResult().code(ACCOUNT_MERGE_NO_ACCOUNT);
        return false;
    }
    ...
    {
        auto otherAccount = hcnet::loadAccount(
