...
using uniform_u64 = hcnet::uniform_int_distribution<uint64_t>;
...
class SlidingWindowTester
{
  ...
  public:
    ...
    {
        mSlidingWindowSample.Seed(hcnet::gRandomEngine());
    }
    ...
    addSamplesAtFrequency(std::chrono::milliseconds timeStep, Args... args)
    {
        ...
        {
            uint64_t sample =
                static_cast<uint64_t>(dist(hcnet::gRandomEngine));
...
sampleFrom(Args... args)
{
    ...
    {
        sample.emplace_back(dist(hcnet::gRandomEngine));
    }
...
testCKMSSample(int const count, Args... args)
{
    ...
    {
        auto x = static_cast<int64_t>(dist(hcnet::gRandomEngine));
        values.push_back(x);
        hist.Update(x);
    }
